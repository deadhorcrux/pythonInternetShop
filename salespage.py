class salespage:
    ...
    def index(self):
        ...
        for c in self.__lib.getSalesCodes():
            ...
            s+='<td><a href = editorm?code=%s>%s</a></td>'%(c,u'edit')
    ...
    def clientCombo(self, code = 0):
        ...
        for c in self.__lib.getClientCodes():
            if (code in self.__lib.getSalesCodes()) and (c == self.__lib.getClientCode(code)):
                v = ' selected'
            else:
                v = ''
    ...
    def salesform(self, code = 0, add = True):
        ...
        s='''<form action = %s method = post>
             <table>
               <tr><td>%s</td><td><input type = number name = client value = '%s'></td></tr>
               <tr><td>%s</td><td>%s</td></tr>
               <tr><td>%s</td><td><input type = number name = product value = '%s'></td></tr>
